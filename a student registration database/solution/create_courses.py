...
with open(input_filename) as f:
	for row in csv.reader(f):
		...
		try:	
			insert_statement = cursor.mogrify("insert into courses values(%s);", (code,))
			cursor.execute(insert_statement)
			insert_statement = cursor.mogrify("insert into course_offerings values(%s, %s, %s, %s, %s);", (code, name, term, instructor, capacity))
			cursor.execute(insert_statement)
			for j in range(len(prerequisites)):
				insert_statement = cursor.mogrify("insert into prerequisites values(%s, %s, %s);", (code, prerequisites[j], term))
				cursor.execute(insert_statement)
